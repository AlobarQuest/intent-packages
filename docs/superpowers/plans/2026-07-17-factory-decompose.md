...
- Produces:
  ...
  - `dry_run_mutation(repo_path: Path, allowed_commands: list[str]) -> set[str]` — `git clone --local` the repo at HEAD into a temp dir, run each command via `subprocess.run(cmd, shell=True, cwd=clone, capture_output=True, text=True, check=True)` (a `CalledProcessError` from a mutator/verifier command is caught and re-raised as `ValidationError` — a command that does not run cleanly *is* a validation failure, and this makes the pip/macOS-sed path degrade to a clean rc=1 instead of a traceback). Fail closed if `git diff --name-only` is empty after the **first** full pass. Run the whole list a **second** time and fail closed if the full **`git diff` content** (not just `--name-only`) changed — a content comparison catches a mutator that rewrites the same file differently each pass, which a name-only comparison would miss. Returns the set of changed repo-relative paths (from `--name-only`, for pin-site coverage). Cleans up the temp clone.
